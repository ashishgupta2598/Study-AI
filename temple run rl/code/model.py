...
class Model(tf.keras.Model):
    def __init__(self, is_new):
        ...
        if is_new: # check if making a new model or loading old (trained) model
           ...
           # self.base_model = ResNet50(input_shape=self.image_shape, include_top=False, weights='imagenet') # resnet50
            self.base_model = EfficientNetB4(input_shape=self.image_shape, include_top=False, weights='imagenet') # resnet50
            ...
            self.model = tf.keras.models.load_model('E:\DL-final-project\models\low_model5') # load the model from saved version
            print(self.model.summary())
    ...
    def train(self, train_ds, epochs):
        ...
        self.model.save('../models/low_model5') # save the model weights
